...
uint8_t *readImage(const char *fileIn, uint32_t *width, uint32_t *height,
                    float ***brightness, uint32_t *unpaddedRowSize,
    ...
    *brightness = (float **)malloc(sizeof(float*) * (*height)); // 2D array to store brightness
    ...
    for (uint32_t i = 0; i < *height; i++)
    {
        for (uint32_t j = 0; j < *unpaddedRowSize; j+=3)
        {
            ...
            (*brightness)[i][k] = ((0.0722*(float)(*pixels)/255) + \
                (0.7152*(float)(*(pixels+1))/255) + \
                (0.2126*(float)(*(pixels+2))/255))*100;
...
uint8_t *sortPixels(uint32_t width, uint32_t height, uint8_t *pixels,
                    float **brightness, uint32_t unpaddedRowSize)
{
    uint8_t *startPixels = pixels;
...
uint8_t *rotateImage(uint32_t *width, uint32_t *height, uint8_t *pixels,
                        uint16_t rotation, uint32_t unpaddedRowSize)
{
        ...
        case 2:
        {
            for (int y = 0; y < *height; y++)
            {
                for (int x = 0; x < *width; x++)
                {
                    ...
                    rotatedPixels[(new_x + new_y * (*width)) * 3] = \
                        pixels[(x + y * (*width)) * 3];
                    rotatedPixels[(new_x + new_y * (*width)) * 3 + 1] = \
                        pixels[(x + y * (*width)) * 3 + 1];
                    rotatedPixels[(new_x + new_y * (*width)) * 3 + 2] = \
...
void writeImage(const char *fileOut, uint32_t width, uint32_t height,
                uint8_t *pixels, uint16_t bitsPerPixel)
{
    ...
    BMPImageHeader imageHeader = {INFO_HEADER_SIZE, width, height, 1,
        bitsPerPixel, 0, imageSize, resolutionX, resolutionY, 0, 0};
