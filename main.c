...
Usage: %s <full path/filein.bmp> <full path/fileout.bmp> <rotation>\n", argv[0]);
        return 0;
    }
    ...
    {
        printf("Incorrect rotation value. \
Rotation value can only be 0, 1, 2 or 3\n");
        return 0;
    }
    uint32_t width, height, unpaddedRowSize;
    float **brightness;
    uint16_t bitsPerPixel;
    uint8_t *pixels = readImage(fileIn, &width, &height, &brightness,
                                &unpaddedRowSize, &bitsPerPixel);
    ...
    {
        pixels = rotateImage(&width, &height, pixels, rotation,
                                unpaddedRowSize);
        writeImage(fileOut, width, height, pixels, bitsPerPixel);
    }
    free(pixels);
